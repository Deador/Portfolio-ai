...
1. Ожидается подтверждение владельцем списка изменений §11 и ответов на вопросы Q1–Q7 из `report/homepage-audit-v2.md`.
2. Нерешённые вопросы прежних сессий (см. ниже): 3-я карточка кейса, навигация карусели, футер, визуальная валидация итерации 3.2 в Storybook.

## Next recommended task

После подтверждения — **Итерация A** («Кейсы», одна смысловая зона): gap16 в карточке кейса + subtitle `--text-s` + реальное изображение 3-й карты (узел `33291:4321`) + удаление orphan `scroll.svg`.

## Suggested prompt for the next session

```texts
Продолжи проект portfolio-ai. Владелец подтвердил отчёт report/homepage-audit-v2.md.
Выполни Итерацию A (только «Кейсы»):
1. HomeCaseCard: обернуть header+subtitle в текстовый блок gap 16 (--spacing-x4); внешний отступ до слота остаётся 32 (--spacing-x8).
2. Subtitle: --text-m → --text-s (16/24, как style_b31ccdd7 в Figma).
3. Заменить плейсхолдер 3-й карты «Редизайн онлайн банка»: выгрузить композит из Figma узла 33291:4321 (тинт rgba(0,177,114,.18) + iphone) @2x в src/content/home/images/, подключить в data.ts.
