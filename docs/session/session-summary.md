...
1. Визуальная QA сегодняшней пачки в браузере: ExpertCard (граница/вращение круга), showreel (адаптивная высота, без кропа), теги «Опыта», кнопки (радиус 16/hover), белый логотип шапки, marquee Medium 500, отступ 160px до «Навыков», карточка «Редизайн онлайн банка» (превью + внешняя ссылка в новой вкладке).
2. Showreel кадр 3: пропорция 1.4439 против 1.5337 у кадров 1–2 — для идеала нужен ассет ~1216×794 от дизайнера.
3. Tag variant `inverted` остался в DS без потребителей после удаления бейджа UI/UX — кандидат на чистку (после удаления тега «Скоро» у Tag.default тоже нет потребителей на главной; в кейсах Tag не используется).
4. Открытые вопросы аудита v2: хедер главной (Q1), H1 24 vs 28px (Q3), флюид заголовков на 320–430 (Q4).
5. Tech debt: латентный баг `Button.pill` (перебивается `.outline/.filled` при равной специфичности); Sass legacy-js-api warnings.
6. Не выполнена **Итерация A** из homepage-audit-v2 («Кейсы»): gap16 title→subtitle, subtitle → `--text-s`, удаление orphan `scroll.svg` (изображение 3-й карты сделано 24.08 по узлу владельца 33321:7415 вместо планового 33291:4321).
7. Дальше по бэклогу: карусель кейсов (линия #B1B1B1 + стрелки 56×56), футер.

## Next recommended task

**Итерация A «Кейсы»** (план из homepage-audit-v2 §14, сокращённый после 24.08): gap16 в текстовом блоке карточки кейса + subtitle `--text-s` + удаление orphan `scroll.svg`. Перед этим — визуальная QA сегодняшних правок владельцем.

## Suggested prompt for the next session

```text
Продолжи проект portfolio-ai. Сначала: визуальная QA главной в браузере (ExpertCard+circle2,
showreel адаптивный, теги Опыта, кнопки CTA/Наверх, логотип шапки, marquee 500, отступ 160px
до Навыков, третья карта кейса — превью mobile-bank и ссылка tilda.ws/case01 в новой вкладке).
Затем Итерация A «Кейсы»:
1. HomeCaseCard: обернуть title+subtitle в текстовый блок gap 16 (--spacing-x4); до слота остаётся 32 (--spacing-x8).
2. Subtitle: --text-m → --text-s (16/24, как style_b31ccdd7 в Figma).
3. Удалить неиспользуемый src/content/home/images/scroll.svg.
