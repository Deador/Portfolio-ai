...
техническое качество; адаптивность/доступность/SEO — вне охвата) проведён двухэтапный аудит. Полный
отчёт по находкам отдан владельцу в чате (не сохранён отдельным файлом в репозитории).

**Предполётная проверка (по ТЗ обязательна):** прод == `HEAD` main на момент старта (`aeb78b7`,
идентичные хеши `index-BcTDYKRD.js`/`index-Cgc14Z7t.css`) — можно было аудировать без оговорок.

**Окружение:** Lighthouse 11 через Node API (`chrome-launcher` + системный Chrome, кастомные
throttling-профили) — медиана 5 прогонов на условие (3 — для дорогого кастомного «плохая сеть»
профиля). `vite-bundle-visualizer` для состава бандла (временная сборка вне репозитория, конфиг не
менялся). Firefox дополнительно проверен и тоже недоступен для автоматизации (Playwright требует
патченую сборку с протоколом Juggler, обычный `firefox.exe` не подходит) — кроссбраузерность
подтверждена только для Chromium.

**Найдено (сводно, P0/P1/P2 — 3/5/6):**
- **P0** `www.trifonovprod.ru` резолвится в GitHub Pages, но TLS падает (`SEC_E_WRONG_PRINCIPAL`) —
  сертификат покрывает только апекс-домен из `CNAME`; легаси DNS-запись `www → deador.github.io`
  осталась с этапа до переезда на кастомный домен. **Решение владельца: пока не трогать.**
- **P0** `http://` не редиректит на `https://`, HSTS нет — настройка «Enforce HTTPS» в GitHub Pages,
  вероятно недоступна из-за проблемы выше. **Решение владельца: пока не трогать.**
- **P0** (переиспользование F2 из адаптивного аудита, усилен этим отчётом) — обе страницы кейсов
  отдают HTTP 404 на прямой заход → Lighthouse/PageSpeed Insights не могут их аудировать вообще
  (`ERRORED_DOCUMENT_REQUEST`, 10/10 попыток). Уточнение: клик по карточке на самой главной НЕ
  вызывает эту проблему (подтверждено точным тестом — реальный SPA client-nav, 0 document-запросов).
  **Решение владельца: смена хостинга — не готовы.**
- **P1** Кеш хешированных JS/CSS/картинок — `max-age=600` (10 минут) вместо `immutable`/годового —
  GitHub Pages не различает хешированные и обычные пути. Решается только сменой хостинга/добавлением
  CDN-прокси — не в этой сессии.
  - **P1 LCP на плохой мобильной сети (400кбит/с/400мс RTT/CPU×4, медиана 3 прогонов): 15.8с** —
  showreel грузил все 3 полноразмерных кадра сразу (1.2 из 1.85МБ веса страницы). **Исправлено, см.
  ниже.**
- **P1** Нет code-splitting — весь JS + `case.json` обоих кейсов в одном чанке (`@remix-run/router`
  34% несжатого бандла, `case.json`×2 = 46KB даже на главной странице, где не нужны). Причина:
  статические импорты `case.json` в `CasePage.tsx` + отсутствие `React.lazy`. **Владелец отклонил**
  предложенный вариант (React.lazy/Suspense даёт заметную заглушку при первом переходе на кейс) — не
  реализовано.
- **P1** При сбое загрузки JS/выключенном JS — полностью пустая белая страница, `<noscript>` не было.
  **Исправлено, см. ниже (частично — только сценарий «JS выключен»).**
- **P1** 38 уязвимостей в зависимостях (`npm audit`, 1 critical) — все в devDependencies (Storybook
  7.6.24, актуальна 8-10.x), прод-бандл содержит только 9 пакетов (react-экосистема), реального риска
  для пользователей нет. **Владелец: оставить как есть** (мажорный апгрейд Storybook — не в этой
  сессии).
- **P2** Заголовки безопасности (CSP/HSTS/X-Frame-Options/Referrer-Policy/Permissions-Policy)
  отсутствуют полностью — GitHub Pages не поддерживает кастомные заголовки, чинится только сменой
  хостинга.
- **P2** Google Fonts грузил неиспользуемый Inter (токен `--font-family-inter` не применяется нигде) —
  **исправлено, см. ниже.** Эффект небольшой: unicode-range не даёт браузеру реально скачивать файлы
  Inter (сетевым перехватом подтверждено — качаются только 4 файла Onest), убрано ради чистоты.
- **P2** HTML: 23 info-уровня W3C Nu Validator (trailing slash на void-элементах, JSX-артефакт) — не
  влияет на работу, фикс не требуется.
- **P2** `npm outdated`: React 18→19, react-router-dom 6.30.4→6.30.6 (патч), остальное — мажорный
  дев-тулинг. Не трогали (мажорные апгрейды — только с отдельного согласия).

**Реализовано (только чистый код, без хостинга/DNS и без React.lazy/Suspense — по прямому запросу
владельца «то что можем сделать через код давай сделаем»):**

1. **Showreel грузит кадры по мере необходимости** (P1, `useShowreel.ts` + `HomeHero.tsx`): добавлено
   состояние `loadedIndices` — изначально открыт только кадр 0, следующий по циклу кадр открывается
   для загрузки сразу, как только текущий становится активным (весь `intervalMs`=4с на фоновую
   загрузку до того, как кадр реально понадобится). `HomeHero` не рендерит `<img>` для ещё не
   открытых кадров — раньше все 3 качались разом при монтировании. При `prefers-reduced-motion`
   предзагрузка кадров 2/3 тоже не запускается (ротации всё равно не будет). Проверено локально
   (`vite preview` + Playwright, перехват сети): было 3 запроса сразу (319+384+530КБ); стало —
   2 сразу (319+384КБ), третий стартует к первой ротации (~4с) с запасом до реального показа (~8с).
   **Экономия ~530КБ на критичном пути первой отрисовки.**
2. **Убран неиспользуемый Inter** (P2) из Google Fonts `<link>` в `index.html`.
3. **`<noscript>`-фолбэк** (P1, частично) в `index.html` — вместо пустого экрана при отключённом JS
   показывается текстовое сообщение. Не закрывает сценарий «JS включён, но не догрузился» (сетевой
   сбой) — это отдельная, более крупная задача (нужно дублировать контент в сыром HTML вне React).
...
**Открыто на будущее:** остальные находки этого отчёта (www/HTTPS/кеш/заголовки безопасности/хостинг —
осознанно отложено владельцем; code-splitting — отклонён предложенный вариант, нужен другой подход
без видимой Suspense-заглушки, если вопрос снова встанет; полный `<noscript>`-сценарий «JS упал после
загрузки»).

### Аудит адаптивности + фикс F1 (горизонтальный скролл шапки) (28.08.2026, ветка fix/responsive-review)

По ТЗ владельца (senior frontend-инженер, независимое ревью адаптивности) проведён двухэтапный
аудит. Полный отчёт по 10 находкам (F1–F10) отдан владельцу в чате этой сессии (не сохранён
отдельным файлом в репозитории — только это резюме).

**Окружение:** Playwright 1.63.0 (`playwright-core`, без загрузки собственных браузеров — сеть
песочницы блокирует `cdn.playwright.dev`) запущен через системный `C:\Program Files\Google\Chrome\
Application\chrome.exe` (`channel:'chrome'`). **WebKit недоступен физически** — ни скачать (та же
сетевая блокировка), ни взять системный (не существует на Windows). Все находки, касающиеся
flexbox/анимаций, помечены «нужна проверка на реальном iPhone/Safari». Lighthouse 11 запущен
локально через `CHROME_PATH` на тот же системный Chrome. Все скрипты/скриншоты/JSON-отчёты — вне
репозитория, в `%TEMP%\claude\...\scratchpad\responsive-audit\` (эфемерная сессионная папка,
недоступна в будущих сессиях).

**Найдено (сводно):**
- **P0 F1** — горизонтальный скролл 320–367px на всех страницах: `.navContainer` в шапке имел
  `flex-shrink:0` даже в мобильном медиа-запросе, из-за чего внутренний `flex-wrap` не срабатывал.
- **P0/P1 F2** — обе страницы кейсов (`/case/acquiring`, `/case/chat`) отдают HTTP 404 на прямой
  заход/reload (побочный эффект GitHub Pages 404-фолбэка из `44d8e88`) → Lighthouse падает с
  `ERRORED_DOCUMENT_REQUEST`, реальным пользователям в браузере с JS не мешает. **Решение
  владельца: пока не трогать** (миграция хостинга — отдельное решение).
- **P1 F3** — переполнение 1025–1151px на страницах кейсов: `ContextSection`/`ProblemSection` —
  фикс-px колонки (`imageSlot:556px`+`cardSlot:520px`, `paragraphBlock:560px`+`citateSlot:460px`) с
  сбросом только на `bp.tablet` (≤1024px). Не утверждено к исправлению.
- **P1 F4** — тап-таргеты шапки 22px (< 24px, WCAG 2.5.8): `.link { font: var(--text-m); padding:0 }`
  → line-height 22.4px без вертикального padding. Не утверждено к исправлению (F1-фикс это не решил).
- **P1 F5** — LCP 14.2s на мобильной главной (Lighthouse, score 69/100): showreel рендерит все 3
  кадра одновременно (`HomeHero.tsx`), 1.2 МБ из 1.68 МБ (89% веса страницы) — картинки. Не
  утверждено к исправлению.
- **P2 F6** — showreel не ставится на паузу на тач (только `onMouseEnter`/`onMouseLeave`, WCAG
  2.2.2). **Решение владельца: пока не трогать.**
- **P2 F7** — console 404 для `favicon.ico`/`apple-touch-icon.png` (файлы всё ещё не получены от
  владельца — известный открытый пункт из предыдущей сессии, подтверждён эмпирически).
- **P2 F8** — тап-таргеты логотип/«Телеграмм» сближены (2px) ровно на 430px — тот же корень, что F1.
- **P2 F9** — `100vh` вместо `dvh` в 4 файлах (`RootLayout`/`CasePage`/`HomePage`/`CaseRenderer`).
- **P2 F10** — Google Fonts CSS блокирует рендер (~883мс–1512мс по Lighthouse), `display=swap` уже
  применён.

**Реализовано (F1, вариант A — единственный утверждённый владельцем):**
- `Header.module.scss`: `.navContainer` внутри `@include bp.mobile` получил `flex-shrink: 1`
  (перебивает базовое `flex-shrink:0`) — на 375px+ и десктопе/таблете поведение не меняется, на
  узких экранах перенос наконец работает.
- Заодно (по отдельному вопросу владельца о номере телефона в шапке): `Header.tsx` получил
  `button2Href`/`button2Target` (по образцу уже существующих `button1Href`/`ctaHref`); `RootLayout.tsx`
  → `button2Href="tel:+79062236634"`. Номер телефона был мёртвым `<button>` без `href`/`onClick` —
  теперь тап на мобильном открывает набор номера.
- Проверено локально на `vite preview` + Playwright/системный Chrome: `scrollWidth === clientWidth`
  на 300–420px на `/` и `/case/acquiring` (было 320–367px); 1280/1440 не затронуты.

Владелец также предложил вариант B (иконка вместо полного текста номера на узких экранах) — не
реализован: в DS нет готовой иконки телефона (`shared/assets/` — только `Logo`/`ArrowUp`/
`ArrowDownRight`), а Figma-коннектор не авторизован в сессии.

Проверки: `type-check` ✅, `lint` ✅, `build` ✅, `build-storybook` ✅.

**Ветка `fix/responsive-review`**: коммит `921f418`, смёржен в `main` merge-коммитом `bd59aeb`,
запушено в `origin/main`.

**Файлы:** `src/shared/ui/organisms/Header/Header.module.scss`,
`src/shared/ui/organisms/Header/Header.tsx`, `src/app/layouts/RootLayout.tsx`.

**Открыто на будущее:** F2–F5, F7–F10 из отчёта выше (F7 самоустранится после загрузки favicon-
файлов владельцем).

### SEO-метатеги (главная + кейсы) и favicon (28.08.2026)

По ТЗ владельца настроены SEO-метатеги и favicon. Правки ограничены `<head>` и статикой —
вёрстка/контент/компоненты (кроме одного точечного `useEffect`, см. ниже) не тронуты.

**Ограничение SPA, согласовано заранее (AskUserQuestion → «Минимально»):** проект — чистый
client-side SPA без SSR/пререндера, GitHub Pages (в т.ч. 404-фолбэк под кастомный домен) отдаёт
один и тот же `index.html` на все роуты. Краулеры мессенджеров (Telegram/WhatsApp/Facebook/VK) не
исполняют JS — значит, шаринг ссылки на `/case/*` всегда покажет OG главной, а не кейса. Google/
Yandex это не касается (их боты рендерят JS). Выбран вариант «Минимально»: статические теги в
`index.html` только для главной + лёгкий JS-хук на кейсах для title/description/OG/Twitter (не даёт
превью в мессенджерах, но корректен для поисковиков и вкладки браузера). Полный фикс (пререндер
отдельных `dist/case/*/index.html`) — отклонён как более рискованный (трогает недавно
стабилизированный GitHub Pages deploy).

- **`index.html`**: `lang="ru"`, title/description/author/canonical, полный блок Open Graph
  (+`og:image:width/height` 1200×630 — добавлено сверх исходного ТЗ для корректного рендера
  превью) и Twitter Card для главной; favicon-теги. Убрана дефолтная иконка `vite.svg`.
- **`CasePage.tsx`**: добавлен `useEffect`, который на страницах кейсов переписывает
  `document.title`, `meta[name=description]`, `og:*`, `twitter:*`, `link[rel=canonical]` и
  восстанавливает значения главной при уходе со страницы. Тексты/превью — в константе `CASE_SEO`
  в этом же файле (не в `case.json` — контент кейсов сознательно не трогали, у существующего
  `case.json.meta` другая формулировка и назначение).
- **og:image кейсов** — по уточнению владельца переиспользованы существующие превью карточек с
  главной (`case-ekvairing.png`, `case-chat-platform.png`, витрина `HomeCasesSection`) вместо
  отдельных заказных ассетов. Обнаружено и сообщено владельцу: реальный размер обоих файлов
  **928×768** (1.21:1) вместо рекомендуемых OG 1200×630 (1.91:1) — при жёстком кропе клиентом
  под 1.91:1 теряется ~37% высоты. Исходники лежат в `src/content/home/images/` и хешируются
  при сборке Vite (подтверждено сборкой) — скопированы в `public/` под стабильными именами
  `og-acquiring.png`/`og-chat.png`, `og:image:width/height` проставлены реальными (928/768).
- **Favicon**: подключены теги `favicon.ico` (`sizes="any"`), `favicon.svg`
  (`type="image/svg+xml"`), `apple-touch-icon.png` (180×180). Владелец выгрузить иконку из Figma
  напрямую агент не смог — коннектор Figma MCP не авторизован в сессии (`plugin:figma:figma`
  требует авторизации через `claude mcp`/`/mcp` или настройки коннекторов claude.ai). Владелец
  загрузил `public/favicon.svg` вручную — путь в `index.html` изначально указывал на `/icon.svg`
  (ошибочное предположение об имени файла), исправлено на фактическое `/favicon.svg`.

**Открыто:** `favicon.ico` и `apple-touch-icon.png` ещё не получены — тег `favicon.ico` указывает
на несуществующий файл (безвредно, современные браузеры используют `favicon.svg`), но на iOS без
`apple-touch-icon.png` при добавлении на экран «Домой» возьмётся скриншот страницы вместо иконки.
Также не получен `og-cover.png` (1200×630) для главной — `og:image` временно указывает на
несуществующий файл.

Проверки: `type-check` ✅, `lint` ✅, `build` ✅ (ассеты `public/` подтверждены в `dist/`),
`build-storybook` ✅.

**Файлы:** `index.html`, `src/app/pages/CasePage.tsx`, `public/favicon.svg` (от владельца),
`public/og-acquiring.png`, `public/og-chat.png` (новые, скопированы из `src/content/home/images/`).
